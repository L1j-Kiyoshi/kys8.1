...
public class NewBossSpawnTable {
	...
	try {
		...
		while (s.hasMoreElements()) {
		    String temp2 = "";
		    StringTokenizer values = new StringTokenizer(s.nextToken(), "出現デー:タイムランダム削除座標範囲グループYNメッセージコメント");
			...
			StringTokenizer mdata = new StringTokenizer(temp2, "秒");
	    ...
	    /*
	     * for (BossTemp temp : bosslist) { temp.NearTime = new
	     * int[temp.SpawnHour.length]; for (int i = 0; i <
	     * temp.SpawnHour.length; i++) { if (temp.rndTime != 0) { int Hour =
	     * temp.SpawnHour[i]; int rndtime = rnd.nextInt(temp.rndTime) + 1;
	     * int Minute = temp.SpawnMinute[i] + rndtime; if (Minute >= 120) {
	     * Hour += 2; Minute -= 120; } else if (Minute >= 60) { Hour++;
	     * Minute -= 60; } StringBuffer NewText = new StringBuffer();
	     * NewText.append(Hour); NewText.append(Minute < 10 ? "0" + Minute :
	     * Minute); temp.NearTime[i] =
	     * Integer.parseInt(NewText.toString().trim()); //
	     * System.out.println("temp.NearTime[i] = " + temp.NearTime[i]); }
	     * else { StringBuffer NewText = new StringBuffer();
	     * NewText.append(temp.SpawnHour[i]);
	     * NewText.append(temp.SpawnMinute[i] < 10 ? "0" +
	     * temp.SpawnMinute[i] : temp.SpawnMinute[i]); temp.NearTime[i] =
	     * Integer.parseInt(NewText.toString().trim()); //
	     * System.out.println("temp.NearTime[i] = " + temp.NearTime[i]);
	     * 
	     * } } }
    ...
    /*
     * 1~6 0 出現デー：月、火、水、木、金、土、日 出現時間：2時00分、5時30分 ランダムタイム：0分 削除タイム：3600秒
     * 出現座標：32726、32832、603 ランダム範囲：0 グループ出現：0 YNメッセージ：1 出現コメント：1
     */
