...
public class Robot {
	...
	if (bot.huntingBot) {
		...
		if (f == 0)
		    ...
		    bot.setTempCharGfx(11379); // フォーノースオークアーチャー
	    } else {
	...
	if (bot.huntingBot) {
		...
		int f = _random.nextInt(4);
		if (f == 0)
		    bot.setTempCharGfx(12702); // フォーノースオーク
		else if (f == 1)
		    bot.setTempCharGfx(12702); // フォーノースオーク
		else if (f == 2)
		...
		if (f1 == 0 && bot.isKnight())
		    ...
		    bot.setTempCharGfx(3889); // 75ランスマスター
		else if (f1 == 3 && bot.getCurrentWeapon() != 20)
		    bot.setTempCharGfx(12702); // 75たデス
		else
		    bot.setTempCharGfx(12702); // 75たデス
	...
	if (bot.getMap().isUnderwater()) {
	    if (!bot.hasSkillEffect(STATUS_UNDERWATER_BREATH)) {
		Broadcaster.broadcastPacket(bot, new S_SkillSound(bot.getId(), 190), true);
		bot.setSkillEffect(STATUS_UNDERWATER_BREATH, 1800 * 1000);
	    }
	}
	if (bot.getMoveSpeed() == 0 && !bot.hasSkillEffect(HASTE) && !bot.hasSkillEffect(L1SkillId.DECAY_POTION)) {
	    bot.setMoveSpeed(1);
	...
	if (bot.isKnight() || bot.isCrown()) {
	    // ディケイないとき
	    if (!bot.hasSkillEffect(L1SkillId.STATUS_BRAVE) && !bot.hasSkillEffect(L1SkillId.DECAY_POTION)) {
		...
		if (!bot.hasSkillEffect(L1SkillId.DANCING_BLADES) && !bot.hasSkillEffect(L1SkillId.SILENCE)) {
		    bot.setSkillEffect(L1SkillId.DANCING_BLADES, (_random.nextInt(50) + 250) * 1000);
		    bot.setBraveSpeed(1);
		...
		if (!bot.hasSkillEffect(L1SkillId.STATUS_ELFBRAVE) && !bot.hasSkillEffect(L1SkillId.DECAY_POTION)) {
	    ...
	    if (!bot.hasSkillEffect(L1SkillId.BLOOD_LUST) && !bot.hasSkillEffect(L1SkillId.SILENCE)) {
	    ...
	    if (percent < 20)
		return false;
	    if (!bot.hasSkillEffect(L1SkillId.MOVING_ACCELERATION) && !bot.hasSkillEffect(L1SkillId.SILENCE)) {
	    ...
	    if (percent < 20)
		return false;
	    if (!bot.hasSkillEffect(L1SkillId.HOLY_WALK) && !bot.hasSkillEffect(L1SkillId.SILENCE)) {
		bot.setSkillEffect(L1SkillId.HOLY_WALK, (_random.nextInt(14) + 50) * 1000);
		bot.setBraveSpeed(4);
		Broadcaster.broadcastPacket(bot, new S_DoActionGFX(bot.getId(), 19));
	...
	if (bot.isKnight()) {
		...
		new L1SkillUse().handleCommands(bot, L1SkillId.REDUCTION_ARMOR, bot.getId(), bot.getX(), bot.getY(),
			null, 192 * 1000, L1SkillUse.TYPE_NORMAL);
		return true;
	    }
	} else if (bot.isCrown()) {
	} else if (bot.isElf()) {
		...
		if (!bot.hasSkillEffect(L1SkillId.STORM_SHOT)) {
		    new L1SkillUse().handleCommands(bot, L1SkillId.STORM_SHOT, bot.getId(), bot.getX(), bot.getY(),
			    null, 960 * 1000, L1SkillUse.TYPE_NORMAL);
		    return true;
		}
	    } else {
		...
		new L1SkillUse().handleCommands(bot, L1SkillId.DRAGON_SKIN, bot.getId(), bot.getX(), bot.getY(), null,
			1200 * 1000, L1SkillUse.TYPE_NORMAL);
		return true;
	    }
	} else if (bot.isDarkelf()) {
		...
		new L1SkillUse().handleCommands(bot, L1SkillId.SHADOW_ARMOR, bot.getId(), bot.getX(), bot.getY(), null,
			960 * 1000, L1SkillUse.TYPE_NORMAL);
		return true;
	    }
		...
		new L1SkillUse().handleCommands(bot, L1SkillId.BURNING_SPIRIT, bot.getId(), bot.getX(), bot.getY(),
			null, 320 * 1000, L1SkillUse.TYPE_NORMAL);
		return true;
	    }
	} else if (bot.isWizard()) {
	}
	...
	if (bot.isCrown()) {
	    clan.addClanMember(bot.getName(), L1Clan.MONARCH, bot.getLevel(), "ロボット", bot.getId(), bot.getType(), 1,
		    bot);
	} else {
	    clan.addClanMember(bot.getName(), 8, bot.getLevel(), bot.getClanMemberNotes(), bot.getId(), bot.getType(),
		    1, bot);
	}
	...
	public void run() {
		...
		if (bot.isDead() || bot._EndThread || L1World.getInstance().getPlayer(bot.getName()) == null)
		    return;
		int npcId = 0;
		int dollType = 0;
		int dollTime = 0;
	...
	public void run() {
		...
		if (bot.isDead() || bot._EndThread || L1World.getInstance().getPlayer(bot.getName()) == null)
		    return;
		poly(bot);
		Broadcaster.broadcastPacket(bot, new S_ChangeShape(bot.getId(), bot.getTempCharGfx()));
	    } catch (Exception e) {
		e.printStackTrace();
	    }
	}
