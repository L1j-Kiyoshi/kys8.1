...
//				System.out.println("■■■■■■■■■■ パプリオンレイド失敗 ■■■■■■■■■■ MAP - " + map);
    ...
    public void addLairUser(L1PcInstance pc) {
        if (!_fafulist.contains(pc)) _fafulist.add(pc);
    }

    /**
     * パプリオンレアに進入していたユーザーかどうかを確認し
     */
    public boolean isLairUser(L1PcInstance pc) {
        return _fafulist.contains(pc);
    }
    ...
    public void RemoveLairUser(L1PcInstance pc) {
        if (_fafulist.contains(pc)) _fafulist.remove(pc);
    }
