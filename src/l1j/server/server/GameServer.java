...
public class GameServer {
	...
	public void initialize() throws Exception {
		...
		System.out.println(" ▶ ラウフル:  " + (rateLawful) + "倍");
		...
		/** 悪魔王狩り場 **/
		if (Config.DEMON_KING_OPERATION) {
			DevilController.getInstance().start();
		}

		/** アデン狩り場 **/
		if (Config.ADEN_HUNTING_OPERATION) {
	...
	private class ServerShutdownThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				world.broadcastServerMessage("安全な場所からログアウトしてください");
	...
	public void shutdown() {
		...
		// manager.savelog();
		// //サーバーダウン時にサーバーログを保存するかどうか、2014年7月12日logディビフォルダ利用時にコメントアウト
