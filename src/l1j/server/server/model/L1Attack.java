...
public class L1Attack {
    ...
    // ●●●● プレイヤーからプレイヤーへの衝突判定 ●●●●
    /*
     * PCへの命中率=（PCのLv +クラス補正+ STR補正+ DEX補正+武器補正+ DAIの枚数/ 2
     * +魔法の補正）×0.68-10これで算出された数値は、自分が最大命中（95％）を与えることができる相手側PCのACそこから相手側PCのACが
	...
	// System.out.println("Final 値 : _hitRate:"+_hitRate+" rnd:"+rnd+"
	// 結果:"+(_hitRate >= rnd));
	// return _hitRate >= rnd;
	// ミスイパック付け
	if (_hitRate >= rnd) {
	...
	// NPCへの命中率=（PCのLv +クラス補正+ STR補正+ DEX補正+武器補正+ DAIの枚数/ 2 +魔法の補正）×5
	// {NPCのAC×（-5）}
	_hitRate = _pc.getLevel();
	...
	if (target_ac != 0) {
	    _hitRate = (100 / target_ac * attacker_lvl); // 被攻撃AC =攻撃Lv
							 // //のとき命中率100％
	} else {
	...
	try {
		...
		if (_weaponType != 20 && _weaponType != 62 && _weaponType2 != 17 && _weaponType2 != 19) { // 弓がない場合はロック発動。
		    if (SkillsTable.getInstance().spellCheck(_targetPc.getId(), L1SkillId.TITANL_ROOK)) {
			...
			if (SkillsTable.getInstance().spellCheck(_targetPc.getId(), L1SkillId.TITANL_BLICK)) {
			    int percent = (int) Math
				    .round(((double) _targetPc.getCurrentHp() / (double) _targetPc.getMaxHp()) * 100);
			    int chance = _random.nextInt(100) + 1;
			    int lockSection = 0;
	...
	/** 祝福書武器ツタ関連 **/
	/*
	 * if (_weaponType != 0) { if (weapon.getBless() == 0 ||
	 * weapon.getBless() == 128) { weaponDamage += 3; } }
	 */
	if (_weaponType != 0) {
		...
		if (_pc.getInventory().checkEquipped(900032)) {
		   ...
		   * if(item.getEnchantLevel() == 7){ Dmgcritical += 1; }else
		   * if(item.getEnchantLevel() == 8){ Dmgcritical += 2; }else
		   * if(item.getEnchantLevel() >= 9){ Dmgcritical += 3; } }
		   */
		/** ヴァラカスの一撃 **/
		if (_pc.getInventory().checkEquipped(22208) || _pc.getInventory().checkEquipped(22209)
		    ...
		    if (chancess <= 8) {
			weaponDamage *= 1.3;
			S_UseAttackSkill packet = new S_UseAttackSkill(_target, _target.getId(), 15841, _targetX,
				_targetY, ActionCodes.ACTION_Attack, false);
		...
		if (_pc.getInventory().checkEquipped(900033)) {
		   ...
		   * if(item.getEnchantLevel() == 8){ Bowcritical += 2; }else
		   * if(item.getEnchantLevel() >= 9){ Bowcritical += 3; } }
		   */
		/** ヴァラカスの一撃 **/
		if (_pc.getInventory().checkEquipped(22208) || _pc.getInventory().checkEquipped(22209)
			|| _pc.getInventory().checkEquipped(22210) || _pc.getInventory().checkEquipped(22211)) {
	...
	if (_weaponType == 54 && _pc.hasSkillEffect(L1SkillId.ASSASSIN)) {
	    if (!_pc.getInventory().checkEquipped(20077) && !_pc.getInventory().checkEquipped(120077)
		    && !_pc.getInventory().checkEquipped(20062)) {
	...
	if (_weaponType != 20 && _weaponType != 62) {
	    dmg = weaponTotalDamage + _statusDamage + _pc.getDmgup() + _pc.getDmgRate()
		    + _pc.get_regist_PVPweaponTotalDamage();
	} else {
	...
	if (_targetPc.hasSkillEffect(COOK_STR) || _targetPc.hasSkillEffect(COOK_DEX)
		|| _targetPc.hasSkillEffect(COOK_INT)) { // リニューアル料理
	    damagereduction += 2;
	}
	...
	/*** 新規レベルの保護 ***/
	/*
	 * if (_calcType == PC_PC) { int castle_id =
	 * L1CastleLocation.getCastleIdByArea(_pc); if (castle_id == 0) { if
	 * (_targetPc.getLevel() < Config.AUTO_REMOVELEVEL || _pc.getLevel() <
	 * Config.AUTO_REMOVELEVEL) { dmg /= 2; _pc.sendPackets(new
	 * S_SystemMessage("新規レベルはダメージの50％だけがかかります。"));
	...
	// キャラクター間ダメージ外部化処理
	/*
	 * if (_calcType == PC_PC) { if (_pc.isCrown()) { dmg +=
	 * Config.PRINCE_ADD_DAMAGEPC; } else if (_pc.isKnight()) { dmg +=
	 * Config.KNIGHT_ADD_DAMAGEPC; } else if (_pc.isElf()) { dmg +=
	 * Config.ELF_ADD_DAMAGEPC; } else if (_pc.isDarkelf()) { dmg +=
	 * Config.DARKELF_ADD_DAMAGEPC; } else if (_pc.isWizard()) { dmg +=
	 * Config.WIZARD_ADD_DAMAGEPC; } else if (_pc.isDragonknight()) { dmg +=
	 * Config.DRAGONKNIGHT_ADD_DAMAGEPC; } else if (_pc.isBlackwizard()) {
	 * dmg += Config.BLACKWIZARD_ADD_DAMAGEPC; } else if (_pc.is戦士()) { dmg
	 * += Config.戦士_ADD_DAMAGEPC; } }
	 */
	/** ACによるダメージ減少修正 **/
	if (dmg <= 0) {
	    _isHit = false;
	...
	if (_pc instanceof L1RobotInstance) {
		...
		if (_pc.getCurrentWeapon() == 46 // 短剣
			|| _pc.getCurrentWeapon() == 20 || _pc.getCurrentWeapon() == 24) {// 弓
	...
	if (_weaponType == 54 && (_random.nextInt(100) + 1) <= (_weaponDoubleDmgChance - weapon.get_durability())
		&& _pc.isDarkelf()) { // 二刀流
	    weaponTotalDamage *= 2.5;
	    // _attackType = 4;
	    _pc.sendPackets(new S_SkillSound(_pc.getId(), 3398));
	...
	/** エンチャントによるツタ関連 **/
	/*
	 * if (_weaponType != 0 && _weaponType != 20) { switch
	 * (weapon.getEnchantLevel()) { case 7: dmg += 1; break; case 8: dmg +=
	 * 2; break; case 9: dmg += 3; break; case 10: dmg += 4; break; case 11:
	 * dmg += 5; break; case 12: dmg += 6; break; case 13: dmg += 7; break;
	 * case 14: dmg += 8; break; case 15: dmg += 9; break; default: break; }
	 * }
	 */
	switch (_weaponId) {
	case 307:
	...
	if (_weaponType == 20) {
	    ...
	    Broadcaster.broadcastPacket(_pc,
		    new S_AttackCritical(_pc, _targetId, _targetX, _targetY, _weaponType, _isHit));// 新たに作成しましてふ
	} else if (_weaponType == 62 && _sting != null) {

	    _pc.sendPackets(new S_AttackCritical(_pc, _targetId, _targetX, _targetY, _weaponType, _isHit));
	    Broadcaster.broadcastPacket(_pc,
		    new S_AttackCritical(_pc, _targetId, _targetX, _targetY, _weaponType, _isHit));
	} else {
	    if (_pc.isWarrior()) {
