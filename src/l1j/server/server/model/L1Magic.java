...
public class L1Magic {
    ...
    public boolean calcProbabilityMagic(int skillId) {
        ...
        if (skillId == CANCELLATION) {
            ...
            if (_calcType == PC_PC && _pc != null && _targetPc != null) {
                ...
                if (_pc.getClanid() > 0 && (_pc.getClanid() == _targetPc.getClanid())
                        && (_pc.get_DuelLine() == _targetPc.get_DuelLine())) {
                ...
                if (_pc.isInParty()) {
                    if (_pc.getParty().isMember(_targetPc)) {
                        _targetPc.sendPackets(
                                new S_SystemMessage("パーティーメンバー" + _pc.getName() + "様がキャンセレーション魔法を詠唱しました。"));
                        return true;
                    }
                }
                // 対象がインビジ状態イルテンキャンセ無効
        ...
        if (_calcType == PC_PC) {
            if (_targetPc.getLevel() < Config.AUTO_REMOVELEVEL || _pc.getLevel() < Config.AUTO_REMOVELEVEL) {
                if (skillId != EXTRA_HEAL && skillId != HEAL && skillId != GREATER_HEAL && skillId != HEAL_ALL
                        && skillId != FULL_HEAL && skillId != NATURES_BLESSING) { // バフ系
        ...
        if (_calcType == PC_PC || _calcType == NPC_PC) {
            if (_calcType == PC_PC) { // スターン中スターンに失敗
                ...
                // _targetPc.hasSkillEffect(BONE_BREAK)) {
            ...
            if (_targetPc.hasSkillEffect(EARTH_BIND)) {
                if (skillId != WEAPON_BREAK && skillId != CANCELLATION // 確率系
                        && skillId != EXTRA_HEAL && skillId != HEAL && skillId != GREATER_HEAL && skillId != HEAL_ALL
                        && skillId != FULL_HEAL && skillId != NATURES_BLESSING && skillId == MANA_DRAIN
                        || skillId == CURSE_PARALYZE || skillId == THUNDER_GRAB || skillId == ERASE_MAGIC
                        || skillId == SHOCK_STUN || skillId == EARTH_BIND || skillId == BONE_BREAK) { // バフ系
                    return false;
                }
            }
        } else {
        ...
        if (_calcType == PC_NPC || _calcType == NPC_NPC) {
            if ((skillId == SILENCE || skillId == AREA_OF_SILENCE) && (_targetNpc.getNpcId() == 45684
                    || _targetNpc.getNpcId() == 45683 || _targetNpc.getNpcId() == 45681
                    || _targetNpc.getNpcId() == 45682 || _targetNpc.getNpcId() == 900011
                    || _targetNpc.getNpcId() == 900012 || _targetNpc.getNpcId() == 900013
                    || _targetNpc.getNpcId() == 900038 || _targetNpc.getNpcId() == 900039
                    || _targetNpc.getNpcId() == 900040 || _targetNpc.getNpcId() == 5096 || _targetNpc.getNpcId() == 5097
                    || _targetNpc.getNpcId() == 5098 || _targetNpc.getNpcId() == 5099
            ...
            if (_calcType == PC_PC) {
                if (_targetPc != null) {
                    if (_targetPc instanceof L1RobotInstance) {
                        if (_targetPc.hasSkillEffect(L1SkillId.ERASE_MAGIC)) {
                            probability = 100;
                            rnd = 100;
                        } else {
                            probability = 20;
                            rnd = _random.nextInt(_targetPc.getResistance().getEffectedMrBySkill() + 1) + 1;
                        }
                    } else {
        ...
        if (_pc != null && _pc.isGm()) {
            _pc.sendPackets(
                    new S_SystemMessage("\\aL> [" + (_targetPc == null ? _targetNpc.getName() : _targetPc.getName())
                            + "][= \\aJ" + msg2 + " \\aL=][" + msg3 + "]"));
        }
        if (_targetPc != null && _targetPc.isGm()) {
            _targetPc.sendPackets(new S_SystemMessage("\\aL < [" + (_pc == null ? _npc.getName() : _pc.getName())
                    + "][= \\aG" + msg2 + " \\aL=][" + msg3 + "]"));
        }
    ...
    private boolean checkZone(int skillId) {
        if (_pc != null && _targetPc != null) {
            if (_pc.getZoneType() == 1 || _targetPc.getZoneType() == 1) {
                if (skillId == CURSE_POISON || skillId == CURSE_BLIND || skillId == WEAPON_BREAK || skillId == SLOW
                        || skillId == CURSE_PARALYZE || skillId == MANA_DRAIN || skillId == DARKNESS
                        || skillId == WEAKNESS || skillId == DISEASE || skillId == SILENCE || skillId == FOG_OF_SLEEPING
                        || skillId == DECAY_POTION || skillId == GREATER_SLOW || skillId == SHOCK_STUN
                        || skillId == ENTANGLE || skillId == ERASE_MAGIC || skillId == EARTH_BIND
                        || skillId == AREA_OF_SILENCE || skillId == WIND_SHACKLE || skillId == POLLUTE_WATER
                        || skillId == STRIKER_GALE || skillId == GUARD_BREAK || skillId == FEAR
                        || skillId == HORROR_OF_DEATH || skillId == ICE_LANCE || skillId == ELEMENTAL_FALL_DOWN
                        || skillId == RETURN_TO_NATURE || skillId == PHANTASM || skillId == CONFUSION
                        || skillId == DESPERADO || skillId == POWEGRRIP) {
                    return false;
                }
            }
        }
    ...
    private int calcProbability(int skillId) {
        ...
        boolean wizardMagic = false; // ウィザードのdebuffかどうか？trueの場合int命中を加算
        ...
        case CANCELLATION:// キャンセ本サーバー魔100に46％ - 魔あたり-1％
            ...
            // _pc.sendPackets(new S_SystemMessage("[確率] -> " + defenseMr + " "
            // + probability + "%"));
        ...
        case TURN_UNDEAD:
            ...
            probability = (int) ((attackInt * 3 + (attackLevel * 2.5) + _pc.getBaseMagicHitUp())
                    - (defenseMr + (defenseLevel / 2)) - 80);
            if (_calcType == PC_PC || _calcType == PC_NPC) {
                if (!_pc.isWizard()) {
                    probability -= 30;
                }
    ...
    public int calcMagicDamage(int skillId) {
        ...
        if (_calcType == PC_PC || _calcType == NPC_PC) {
            ...
            if (SkillsTable.getInstance().spellCheck(_targetPc.getId(), L1SkillId.TITANL_MAGIC)) {
        ...
        if (_calcType == PC_PC) {
            if (_targetPc.getRobotAi() != null && (_targetPc.noPlayerCK || _targetPc.noPlayerck2 || _targetPc.isGm())) {
                if (_targetPc != null && _targetPc.getClanid() != 0
                        && !_targetPc.getMap().isSafetyZone(_targetPc.getLocation())) {
...
//        }
    ...
    private int calcPcMagicDamage(int skillId) {
        ...
        if (_targetPc.hasSkillEffect(COOKING_1_0_S) || _targetPc.hasSkillEffect(COOKING_1_1_S)
                ...
                || _targetPc.hasSkillEffect(COOKING_1_6_S) || _targetPc.hasSkillEffect(COOKING_1_8_S)
                || _targetPc.hasSkillEffect(COOKING_1_9_S) || _targetPc.hasSkillEffect(COOKING_1_10_S)
                || _targetPc.hasSkillEffect(COOKING_1_11_S) || _targetPc.hasSkillEffect(COOKING_1_12_S)
                || _targetPc.hasSkillEffect(COOKING_1_13_S) || _targetPc.hasSkillEffect(COOKING_1_14_S)
