...
public class L1War {
	...
	public void handleCommands(int war_type, String attack_clan_name,
		...
		if (war_type == 1) {
			...
			_castle = GetCastle();
			if (_castle != null) {
				_warEndTime = LongType_setTime(_castle.getWarTime()
						.getTimeInMillis(), Config.ALT_WAR_TIME_UNIT,
						Config.ALT_WAR_TIME);
				...
				if (clan != null) {
					...
					for (int k = 0; k < clan_member.length; k++) {
						if (clan_member[k] != null)
							clan_member[k].sendPackets(new S_War(3,
									clanList[j], defence_clan_name));
					}
				}
			}
		}
